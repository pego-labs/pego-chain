...
const (
	...
	delegatorRewardPercent = 70 // it means 70% percentage of gas fee and block reward incoming will be distributed to delegator
...
var (
	...
	systemContracts = map[common.Address]bool{
		...
		common.HexToAddress(systemcontracts.NftPnnContract):    true,
		common.HexToAddress(systemcontracts.PgvtContract):      true,
	}
)
...
// Parlia is the consensus engine of PEGO
type Parlia struct {
...
func New(
	...
	daoABI, err := abi.JSON(strings.NewReader(systemcontracts.SystemDaoABI))
	if err != nil {
		panic(err)
	}
	c := &Parlia{
...
func (p *Parlia) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	snap, err := snap.apply(headers, chain, parents, p.chainConfig.ChainID, p.chainConfig.CommunitySwapBlock)
	if err != nil {
		return nil, err
	}
...
func (p *Parlia) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	// Set gas price
	baseGasPrice, err := p.getBaseGasPrice(header.ParentHash, new(big.Int).Sub(header.Number, common.Big1))
	if err != nil {
		return err
	}
...
func (p *Parlia) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction,
	...
	if p.chainConfig.IsUnderCommunitySwap(header.Number) && header.Number.Uint64()%p.config.Epoch == 0 {
		newValidators, err := p.getCurrentValidators(header.ParentHash, new(big.Int).Sub(header.Number, common.Big1))
		if err != nil {
			return err
		}
...
func (p *Parlia) getBaseGasPrice(blockHash common.Hash, blockNumber *big.Int) (*big.Int, error) {
	// block
	blockNr := rpc.BlockNumberOrHashWithHash(blockHash, false)

	// method
	method := "getBaseGasPrice"
...
// getBlockRewards get current dao block miner reward
...
// getBlockRewards get current dao block miner reward
func (p *Parlia) getDelegatorRewardPercent(blockHash common.Hash, blockNumber *big.Int) (*big.Int, error) {
	// block
	blockNr := rpc.BlockNumberOrHashWithHash(blockHash, false)

	// method
	method := "getDelegatorRewardPercent"
	...
	data, err := p.systemDaoABI.Pack(method)
	if err != nil {
		log.Error("Unable to pack tx for getDelegatorRewardPercent", "error", err)
		return big.NewInt(0), err
	}
	// call
	msgData := (hexutil.Bytes)(data)
...
// getBlockRewards get current dao block miner reward
func (p *Parlia) getMintPg(blockHash common.Hash, blockNumber *big.Int) (*common.Address, *big.Int, error) {
	// block
	blockNr := rpc.BlockNumberOrHashWithHash(blockHash, false)
	...
	type ReceivedAddrR struct {
		addr   *common.Address
		amount *big.Int
	}

	//receivedMap := []interface{}{}
	...
	ret2 := new(*big.Int)
	//err := p.systemDaoABI.UnpackIntoInterface(&[]interface{}{ret1, ret2},method, result)
	if err := p.systemDaoABI.UnpackIntoInterface(&[]interface{}{ret1, ret2}, method, result); err != nil {
		return nil, big.NewInt(0), err
	}
...
func (p *Parlia) isBlackAddress(blockHash common.Hash, blockNumber *big.Int, addr common.Address) (bool, error) {
	// block
	blockNr := rpc.BlockNumberOrHashWithHash(blockHash, false)
...
func (p *Parlia) initContract(state *state.StateDB, header *types.Header, chain core.ChainContext,
	...
	contracts := []string{
		...
		systemcontracts.NftPnnContract,
		systemcontracts.GovernorContract,
		systemcontracts.PgvtContract,
	}
	// get packed data
	data, err := p.systemDaoABI.Pack(method)
	if err != nil {
...
//	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
//	// get system message
//	msg := p.getSystemMessage(header.Coinbase, common.HexToAddress("0xfFf138c7774E977771fAEbb59eb28D4f50425375"), nil, amount)
//	// apply message
