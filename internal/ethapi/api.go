...
func (s *PrivateAccountAPI) SendTransaction(ctx context.Context, args TransactionArgs, passwd string) (common.Hash, error) {
	...
	if args.GasPrice != nil {
		if head := s.b.CurrentHeader(); head.BaseFee != nil {
			if args.GasPrice.ToInt().Cmp(head.BaseFee) < 0 {
				return common.Hash{}, errors.New("GasPrice too low")
			}
		}
	}
...
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	...
	if tx.GasPrice().Cmp(b.CurrentHeader().BaseFee) < 0 {
		return common.Hash{}, errors.New("gas price is too low")
	}
	...
	//check sender is in black
	f, err := isBlackAddress(from, b)
	if err != nil {
		return common.Hash{}, err
	}
	if f {
		return common.Hash{}, errors.New("address is in black so can not send transaction")
	}
...
func isBlackAddress(addr common.Address, b Backend) (bool, error) {
	// block
	blockNr := rpc.BlockNumberOrHashWithHash(b.CurrentHeader().Hash(), false)
	...
	data, err := systemDaoABI.Pack(method, addr)
	if err != nil {
	...
	result, err := ethAPI.Call(ctx, TransactionArgs{
		Gas:  &gas,
		To:   &toAddress,
		Data: &msgData,
	}, blockNr, nil)
	...
	var ret0 bool
	if err := systemDaoABI.UnpackIntoInterface(&ret0, method, result); err != nil {
		return false, err
	}
	return ret0, nil
}
